...
num_jobs_nnet="2 2"
#num_jobs_nnet="2 2 2"    # Number of neural net jobs to run in parallel.  This option
                       # is passed to get_egs.sh.  Array must be same length
                       # as number of separate languages.
#################################
am_weight="0.8 0.2"
#am_weight="1 0.8 0.2"  # for contring weight between each language
################################
...
mix_up="0 0 0" # Number of components to mix up to (should be > #tree leaves, if
...
###################################################################################################################################################
    ...
    weights_change=$(echo $am_weight | sed 's/ /:/g') # get as colon separated list.
    ...
    $cmd $dir/log/average.$x.log \
      nnet-am-average --weights=$weights_change --skip-last-layer=true \
